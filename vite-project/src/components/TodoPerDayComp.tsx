...
const TodoPerDayComp = ({todo, todos, setTodos}: PropsTodoType): JSX.Element => {
  ...
  const handleEdit = (e: React.FormEvent, id: number): void => {
    e.preventDefault();
    setTodos(todos.map(todo => todo.id === id ? { ...todo, todo: todo.date } : todo))
        ...
        <div className="daycomp-box">
          ...
          <div className="input-button-container">
            ...
            <div>
              ...
              <button type="button" onClick={(e) => handleEditProject(e, todo.id)} className="modify-btn"><BsPencilSquare size={18} /></button>
            </div>
          </div>
        </div>
