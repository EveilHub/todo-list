...
// Test de la fonction handleChangePriority
describe("handleChangePriority", () => {
  it("should update priority and call API", () => {
    let todos = [...initialTodos];
    const setTodos = (fn: any) => { todos = fn(todos); };
    let paramsPriority = { hidePriority: false, bgColor: "blue" };
    const setParamsPriority = (fn: any) => { paramsPriority = fn(paramsPriority); };

    //const e = { target: { value: "high" } };

    const e = {
      target: { value: "high" },
      nativeEvent: {}, // Événement natif simulé
      currentTarget: { value: "high" },
      bubbles: true,
      cancelable: true,
    ...
    expect(callApiPriority).toHaveBeenCalledWith("1", "high");
    expect(paramsPriority.hidePriority).toBe(true);
  });
});

// Test de la fonction callChangeDay
describe("callChangeDay", () => {
  it("should update selectedDay and call API", () => {
    let todos = [...initialTodos];
    const setTodos = (fn: any) => { todos = fn(todos); };
    let dayBool = false;
    const setDayBool = (fn: any) => { dayBool = fn(dayBool); };

    //const e = { target: { value: "tuesday" } };

    const e = {
      target: { value: "lundi" },
      nativeEvent: {},
      currentTarget: { value: "lundi" },
      bubbles: true,
      cancelable: true,
    } as ChangeEvent<HTMLSelectElement>;
...
//   it("should handle error in callApiPriority", () => {
//     const error = new Error("API failure");
//     callApiPriority.mockRejectedValueOnce(error); // simule un échec d'API
...
//     expect(() => {
//       handleChangePriority(e, "1", setTodos, setParamsPriority);
//     }).not.toThrow(); // vérifie qu'il n'y a pas d'exception levée
//     expect(callApiPriority).toHaveBeenCalledWith("1", "option2");
//     expect(paramsPriority.hidePriority).toBe(true);
//   });
